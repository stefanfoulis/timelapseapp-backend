...
class S3Storage(storages.backends.s3boto3.S3Boto3Storage):
    ...
    def __init__(self, dsn):
        ...
        endpoint_url = furl.furl().set(scheme=protocol, host=dsn.host, port=dsn.port)
    ...
    def _get_key_name(self, name):
        ...
        return posixpath.normpath(
            posixpath.join(self.location, name.replace(os.sep, "/"))
        )
...
class _DSNSettingConfiguredStorage(LazyObject):
    ...
    def _setup(self):
        ...
        if not dsn:
            ...
            storage_class = django.core.files.storage.get_storage_class(
                SCHEMES[backend_name]
            )
...
def dsn_setting_configured_storage(setting_name):
    ...
    return type(
        "DSNSettingConfiguredStorage",
        (_DSNSettingConfiguredStorage,),
        {
...
def dsn_variable_configured_storage(dsn):
    ...
    return type(
        "DSNVariableConfiguredStorage",
        (_DSNVariableConfiguredStorage,),
        {"_dsn": dsn, "_deconstructor": lambda self: (path, [dsn], {})},
    )()
...
def upload_to_movie_rendering(instance, filename):
    ...
    return "movies/{camera}/{size}/{filename}".format(
        size=instance.size, filename=filename, camera=instance.movie.camera.name
    )
